...
def main(options):
    ...
    try:
        while True:
            ready = select.select([sock], [], [], 1) # Wait until read for read or timeout.
            if ready[0]:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('network_discovery', description='Tries to find a service matching the MAGIC_NUMBER on the local network.')

    parser.add_argument('-b', '--broadcast-port', help='The port to broadcast to. Default 55009.', default=55009)
    parser.add_argument('-x', '--exit-if-found', help='Exit the program when the first service is found.', default=True)
